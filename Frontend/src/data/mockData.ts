import { User, Course, FeedbackForm, Analytics, StudentData } from '../types';
...
const generateStudents = (): StudentData[] => {
...
export const studentsData: StudentData[] = generateStudents();
...
const generateCourses = (): Course[] => {
  ...
  departments.forEach((dept) => {
    const deptCode = dept.substring(0, 2).toUpperCase();
    const deptCoursesData = departmentCourses[dept as keyof typeof departmentCourses];
    ...
    for (let year = 1; year <= 4; year++) {
      for (let semType = 0; semType < 2; semType++) {
        ...
        if (dept === 'Computer Science') {
          semesterCourses = (deptCoursesData as any)[semKey] || [];
        } else {
          // For other depts, use year-based courses
          semesterCourses = (deptCoursesData as any)[year] || [];
        }
        ...
        semesterCourses.forEach((courseName: string, idx: number) => {
